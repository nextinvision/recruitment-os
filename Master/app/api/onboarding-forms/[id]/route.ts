...
import { getOnboardingFormById, updateOnboardingForm, deleteOnboardingForm } from '@/modules/onboarding-forms/service'
...
export async function DELETE(
  ...
  try {
    ...
    const { id } = await params
    await deleteOnboardingForm(id)
    const response = NextResponse.json({ message: 'Form deleted' }, { status: 200 })
    const origin = request.headers.get('origin')
    return addCorsHeaders(response, origin)
  } catch (error) {
