 ...
 * Fetches jobs from various APIs (Google, Adzuna, JobSpy, SerpApi, etc.)
...
interface JobFetchOptions {
  query?: string
  location?: string
  source: 'GOOGLE' | 'ADZUNA' | 'GOOGLE_STRUCTURED' | 'JOBSPY'
  limit?: number
  recruiterId?: string
...
export class JobFetchService {
  ...
  private mapJobSpyRowToFetchedJob(row: Record<string, unknown>): FetchedJob {
    ...
    const currency = (row.currency as string) || 'INR'
    if (min && max) {
    ...
    // Map job_type to JobType enum if possible
    let jobType: JobType = JobType.ONSITE
    const rawType = String(row.job_type || '').toLowerCase()
    if (rawType.includes('remote')) jobType = JobType.REMOTE
    else if (rawType.includes('hybrid')) jobType = JobType.HYBRID
    else if (row.is_remote === true) jobType = JobType.REMOTE
  ...
  async fetchAll(options: JobFetchOptions): Promise<FetchedJob[]> {
    const sources = options.source === 'GOOGLE'
      ...
      : options.source === 'ADZUNA'
        ? ['ADZUNA']
        : ['GOOGLE', 'ADZUNA'] // Fetch from all if not specified
